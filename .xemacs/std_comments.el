...
;;(global-set-key "^Ch" 'update-std-header)
;;(global-set-key "^C^D" 'do_insert_time)
;;(global-set-key "^C^H" 'std-file-header)
(global-set-key "\C-cf" 'update-std-header)
(global-set-key "\C-cd" 'do_insert_time)
;;(global-set-key "\C-c\C-f" 'std-file-header)
(global-set-key [f12] 'std-file-header)
...
;; Last update Wed Jun  9 20:34:00 2010 Gary van Woerkens
      header-for        " for "
      header-in         " in "
      domaine-name      "chewam.com")
...
(add-hook 'cperl-mode-hook
          '(lambda ()
             (define-key cperl-mode-map "^C^C"
               'comment-region)
             (define-key cperl-mode-map "^C^Hh"
               'std-file-header)))
